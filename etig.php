...
define('ALLOWED_EXTENSIONS', ['txt', 'php', 'html', 'css', 'js', 'json', 'xml', 'log', 'md']);
...
define('STRICT_MIME_CHECK', true); // Set to true for strict MIME validation
...
function validateFileContent($filePath) {
    // If strict MIME check is disabled, only validate by extension
    if (!STRICT_MIME_CHECK) {
        return true;
    }
    ...
    // Also allow if it starts with 'text/' or 'image/'
    if (strpos($mimeType, 'text/') === 0 || strpos($mimeType, 'image/') === 0) {
        return true;
    }
    
    if (!in_array($mimeType, $allowedMimes)) {
        auditLog('SECURITY_VIOLATION', 'Invalid file content type: ' . $mimeType . ' for ' . basename($filePath));
        return false;
    }
...
if (isset($_FILES['standard_upload']) && $_FILES['standard_upload']['error'] !== UPLOAD_ERR_NO_FILE) {
    if (!checkRateLimit()) {
        auditLog('RATE_LIMIT', 'Upload rate limit exceeded');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['remove']) || isset($_POST['bulk_delete']) || isset($_POST['old_name'])) {
        if (!checkRateLimit()) {
            auditLog('RATE_LIMIT', 'File operation rate limit exceeded');
...
<div class="container">
    ...
    <?php if ($currentDirectory !== BASE_DIR): ?>
        <form method="post">
            <button name="navigate" value="<?= dirname($currentDirectory) ?>" class="btn" style="margin-bottom: 12px;">← Parent Directory</button>
        </form>
    <?php endif; ?>
        ...
        ?>
            <tr>
                ...
                <td>
                    ...
                    <?php if ($isDirectory): ?>
                        <form method="post">
                            <button name="navigate" value="<?= htmlentities($fullPath) ?>" class="btn btn-sm">Open</button>
                        </form>
                    <?php else: ?>
                        <form method="post">
                            <button name="view" value="<?= htmlentities($item) ?>" class="btn btn-sm">View</button>
                        </form>
                        <form method="post">
                            <button name="edit" value="<?= htmlentities($item) ?>" class="btn btn-sm">Edit</button>
                        </form>
                    <?php endif; ?>
                        <form method="post">
                            <button name="download" value="<?= htmlentities($item) ?>" class="btn btn-sm">Download</button>
                        </form>
                        <form method="post">
                            <button name="rename" value="<?= htmlentities($item) ?>" class="btn btn-sm">Rename</button>
                        </form>
                        <form method="post">
                            <input type="hidden" name="csrf_token" value="<?= generateCSRFToken() ?>">
                            <button name="remove" value="<?= htmlentities($item) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this item?')">Delete</button>
                        </form>
                    </div>
                </td>
            </tr>
